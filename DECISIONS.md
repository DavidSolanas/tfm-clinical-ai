...
### 2026-06-04 — Full-context QLoRA training on a rented cloud GPU; inference stays local

**Decision:** Fine-tune at `max_seq_length=14336` (covers every SFT sample — dataset max is ~12,601 Llama tokens — with zero truncation) on a rented **RunPod RTX 3090 (24 GB)**, batch=2 × grad_accum=4 (effective batch 8, unchanged), **bf16** precision. Convert **both** the base and the FT-merged model to `q4_k_m` GGUF **in the same pod session** (one llama.cpp build, so base and FT differ only in LoRA weights). All inference and the ablation evaluation continue to run **locally** on the 10 GB card via llama.cpp, served at `--ctx-size 14336` (a paired decision — see Consequences). MLflow continuity uses a **file store on the pod's persistent volume**, copied back and `import-run` into the local SQLite tracking server.

**Context:** The SFT user message (clinical note + retrieved PubMed evidence) is long: p50 ≈ 3,540 tokens, p95 ≈ 6,118, max ≈ 12,601. On the 10 GB card the largest non-truncating context is ~4,096 tokens, which truncates ~30 % of training samples — and the truncated part is the **retrieved evidence**, not the assistant response. That systematically depresses the RAG-context-dependent RAGAS metrics (`faithfulness`, `context_recall`) for configs B and D, confounding the central ablation (a reviewer could not tell whether a result reflects the model or training-time truncation). Training memory ≫ inference memory (optimizer states + gradients + full activation graph vs. weights + KV cache), so full-context training needs >10 GB while full-context *inference* of a 4-bit model fits 10 GB. The privacy requirement **RNF-01** (`plantilla.tex`) is explicitly scoped to *inference* ("durante la inferencia"); `PROPOSAL.md` already contemplated cloud (Colab) training. Training data is public, de-identified MTSamples + synthetic teacher output, so no private data leaves local hardware.

**Alternatives considered:**
- **Train locally at 4096 (status quo):** zero cost, narrative purity, no new moving parts. Rejected: the ~30 % evidence-truncation confound undermines the headline ablation.
- **Train locally at 2048:** worse — covers <10 % of samples.
- **Rent cloud GPU, train at 14336 (chosen):** ~$2–4 of a $25 budget, removes the confound, consistent with RNF-01's inference-only scope.
- **`q8_0` GGUF for eval fidelity:** rejected — `q8_0` (~8.5 GB) + a 14k KV cache (~1.8 GB) exceeds the 10 GB eval budget. `q4_k_m` (~4.9 GB) fits with headroom and matches the existing "4-bit inference" decision (2026-03-24). `q5_k_m` is the fallback if `q4_k_m` quality is poor.
- **Base GGUF from the unsloth HF repo vs. same-session conversion:** the HF repo is easy but introduces quant-recipe and chat-template-metadata drift between base and FT. Same-session conversion makes them toolchain-identical (~10 min on a rental already running). Chosen for ablation validity.
- **Live MLflow logging via SSH reverse tunnel vs. file store on the persistent volume:** the tunnel logs live (real-time loss, no reconciliation) but has multiple live failure points (tunnel drop, laptop sleep, network blip) that gap or lose tracking. The file store writes to physical disk every step and survives all of those, at the cost of one `import-run` step. Chosen for durability; live loss can be watched via training stdout anyway.

**Rationale:** Cloud training is a one-time **offline** step whose only purpose is to remove a data-quality confound; it does not touch the deployment path, so the "private, local, consumer-hardware inference" thesis claim is preserved intact (and is now glossed in `CONTEXT.md` as the train-time vs. inference-time compute split). The cost is negligible against budget; the methodological gain (untruncated evidence in every training example) is what a thesis examiner cares about. `bf16` (available on Ampere) avoids the fp16 loss-scaling NaN risk over a paid long-context run.

**Consequences:**
- `configs/training.yaml` canonical values become `max_seq_length=14336`, `per_device_batch_size=2`, `gradient_accumulation_steps=4`. **Local 10 GB training requires overriding** to `--max-seq-length 4096 --batch-size 1 --grad-accum 8` (documented inline).
- `src/training/finetune.py` auto-detects `bf16` via `torch.cuda.is_bf16_supported()` (falls back to fp16 where unsupported) and logs the chosen precision to MLflow.
- New `scripts/convert_to_gguf.py` produces the base (`--base-only`) and FT (`--adapter <dir>`) GGUFs; `run_finetune.py` still only saves the LoRA adapter.
- **Paired serving constraint:** the eval llama.cpp server **must** run `--ctx-size 14336`. Serving at a shorter context would re-truncate the same long prompts at eval time, producing a train/serve mismatch worse than uniform truncation. KV cache at 14k (~1.8 GB) + `q4_k_m` weights (~4.9 GB) fits the 10 GB card.
- MLflow host port corrected to **5001** across the repo (`docker-compose.yml` publishes `5001:5000`; `.env.example`, `src/tracking.py`, `scripts/verify_setup.py` previously defaulted to 5000 — a latent bug for local runs too).
- Reproducibility is **procedural** (pinned config + `seed=42` + dataset hash + MLflow-logged hyperparameters), not bit-exact across hardware — the honest claim when changing GPUs.
- Thesis Cap. 5/6: present the train-in-cloud / deploy-locally split as methodological rigor, and refine the 2026-03-24 "fits in 10 GB incl. fine-tuning" statement (true at 4k context; full-context training is offloaded as a one-time step).
- **Not version-controlled:** the GGUFs, the LoRA adapter, and the pod's `mlruns/` are large/derived artifacts — keep them out of git (`data/`, `models/` are already untracked).
