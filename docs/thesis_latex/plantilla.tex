...
\citeA{gu_pubmedbert_2021} demostraron que el preentrenamiento \textit{desde cero} sobre PubMed (en lugar de adaptar un modelo BERT general) produce representaciones superiores en el benchmark BLURB, que agrega 8 tareas de NLP biomédico. PubMedBERT (y su variante optimizada para abstracts, \textit{BiomedNLP-BiomedBERT-base-uncased-abstract}) es el modelo seleccionado como embedder en el pipeline RAG de este trabajo, dado que el corpus de recuperación está compuesto íntegramente por abstracts de PubMed. \citeA{fang2023bioformerefficienttransformerlanguage} presentaron Bioformer, un modelo 40\% más pequeño que PubMedBERT y mantiene una diferencia de rendimiento inferior al 1\% en los mismos benchmarks, lo que lo convierte en una alternativa válida si las restricciones computacionales priorizan la velocidad de inferencia sobre la precisión marginal.
...
\textbf{Fase 2: Construcción del índice vectorial.} Los abstracts de PubMed se fragmentan en chunks y se codifican con el modelo de embeddings \textit{BiomedNLP-BiomedBERT-base-uncased-abstract} \cite{gu_pubmedbert_2021}. Los vectores se indexan en Qdrant con distancia coseno y metadatos de año y especialidad.
...
Embeddings & BiomedNLP-BiomedBERT-base & Preentrenado en abstracts PubMed; óptimo para recuperación biomédica \\
Vector DB & Qdrant (Docker) & Soporte HNSW + filtrado por metadatos; API REST sencilla \\
Orquestación RAG & LlamaIndex & Framework validado en entornos clínicos \cite{rau_context-based_2023} \\
