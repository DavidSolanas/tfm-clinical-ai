...
El objetivo general (Capítulo~\ref{cap:objetivos}) se ha alcanzado: se ha construido y evaluado el sistema híbrido y se ha cuantificado la contribución diferencial de cada componente. El grado de consecución de los objetivos específicos es el siguiente:

\begin{itemize}
  \item \textbf{OE1 (pipeline de ingestión e indexación semántica).} Cumplido. Se ha construido un corpus de \textit{abstracts} de PubMed dirigido por el árbol de descriptores MeSH e indexado en Qdrant mediante búsqueda híbrida (denso~+~BM25 con fusión RRF), con embeddings biomédicos seleccionados empíricamente (Bioformer-16L) sobre un proxy de categoría MeSH (Secciones~\ref{sec:c5_embedding} y~\ref{sec:c5_rag}).
  \item \textbf{OE2 (ajuste fino con QLoRA sobre MTSamples).} Cumplido. Se ha ajustado LLaMA-3.1-8B con QLoRA sobre un conjunto de instrucción-respuesta derivado de MTSamples mediante un \textit{teacher} local, adaptando estilo, terminología y estructura de la respuesta al registro clínico (Secciones~\ref{sec:c5_sft} y~\ref{sec:c5_qlora}).
  \item \textbf{OE3 (integración híbrida bajo MLOps).} Cumplido. Ambos módulos se integran en una arquitectura única, con seguimiento de las cuatro configuraciones ablativas en MLflow y reproducibilidad procedimental (configuración fijada, \texttt{seed=42}, \textit{hash} de dataset e hiperparámetros registrados).
  \item \textbf{OE4 (evaluación de las cuatro configuraciones).} Cumplido. El Capítulo~\ref{cap:evaluacion} reporta las métricas RAGAS y un conjunto de métricas deterministas de comportamiento de citación sobre una muestra pareada de 50~consultas, con intervalos de confianza \textit{bootstrap}.
  \item \textbf{OE5 (determinar la ventaja del sistema híbrido).} Cumplido. La configuración~D domina a las configuraciones parciales en todos los ejes medidos, y el análisis identifica qué limitación específica resuelve cada técnica: RAG aporta evidencia y trazabilidad, el ajuste fino aporta alineamiento de formato y relevancia.
\end{itemize}

En cuanto a los criterios cuantitativos de éxito asociados al requisito RNF-07, conviene reportarlos sin redondeos favorables. El criterio sobre \textit{answer relevancy} (que la configuración~D supere a la base en al menos $0{,}10$~puntos) \textbf{se cumple con amplio margen} ($+0{,}569$). El criterio sobre \textit{faithfulness} requiere la matización ya documentada en la Sección~\ref{sec:c6_discusion}: esta métrica no está definida para la configuración base (sin contexto recuperado), por lo que su único eje comparable es C$\to$D, sobre el que el ajuste fino mejora la fidelidad en $+0{,}083$~puntos. La dirección y el signo del efecto respaldan la hipótesis, si bien su magnitud queda ligeramente por debajo del umbral de $0{,}10$ que se barajó en una formulación inicial del criterio.

\section{Contribuciones del trabajo}
\label{sec:c7_contribuciones}

Las contribuciones concretas de este trabajo, en línea con la brecha identificada en la literatura (Sección~\ref{sec:c2_sintesis}), son:

\begin{itemize}
  \item \textbf{Un estudio ablativo aplicado a la asistencia bibliográfica clínica sobre literatura primaria de PubMed}, realizado íntegramente sobre un único modelo de código abierto cuantizado en hardware modesto, frente al patrón dominante de modelos propietarios y arquitecturas multi-LLM \cite{amugongo_retrieval_2025, zakka_almanac_2024, lozano_clinfoai_2023}.
  \item \textbf{Una familia de métricas deterministas de comportamiento de citación}, libres de juez y reproducibles, que cuantifican la activación, el formato y el anclaje de las citaciones, y que revelan modos de fallo (como la fabricación de la forma de una citación) invisibles a las métricas de adherencia de formato y a las métricas RAGAS estándar.
  \item \textbf{Un mecanismo explícito de verificación automática de citaciones y de abstención}, que permite medir directamente la contribución de RAG a la trazabilidad y a la contención de la alucinación de identificadores.
  \item \textbf{Una herramienta funcional integrada} (consola de evidencia clínica con interfaz web Gradio) que materializa la configuración híbrida completa para su uso interactivo.
\end{itemize}

\section{Limitaciones}
\label{sec:c7_limitaciones}

Más allá de las amenazas a la validez del estudio empírico ya detalladas en la Sección~\ref{sec:c6_limitaciones} (tamaño muestral de 50~consultas, ausencia de un \textit{benchmark} de recuperación con anotación de oro, techo del \textit{context recall} inducido por la referencia del \textit{teacher} y reproducibilidad procedimental y no bit a bit), procede señalar las limitaciones de alcance del sistema en su conjunto:

\begin{itemize}
  \item \textbf{Cobertura del conocimiento.} La base de evidencia se limita a \textit{abstracts} de PubMed, no a texto completo, lo que acota la profundidad de la evidencia recuperable.
  \item \textbf{Validación de usabilidad.} La evaluación de usabilidad es una revisión heurística del autor (Sección~\ref{sec:c6_usabilidad}); carece de validación con profesionales sanitarios y de métricas de interacción.
  \item \textbf{Idioma.} El sistema opera en inglés, lo que limita su aplicabilidad directa en entornos clínicos hispanohablantes.
  \item \textbf{Alcance asistencial.} El sistema es una ayuda a la consulta bibliográfica y no un sistema de soporte a la decisión clínica; no debe interpretarse como sustituto del juicio profesional.
\end{itemize}

\section{Líneas de trabajo futuro}
\label{sec:c7_futuro}

Las limitaciones anteriores delimitan las líneas de continuación más prometedoras:

\begin{itemize}
  \item \textbf{Evaluación de usabilidad con usuarios reales.} Línea prioritaria: realizar una sesión de evaluación con al menos un profesional del dominio clínico sobre la interfaz integrada, con métricas de interacción (tiempo de tarea, tasa de éxito, satisfacción percibida) que complementen la revisión heurística.
  \item \textbf{Refinamiento de la interfaz.} Subsanar la reserva detectada en la revisión heurística (Sección~\ref{sec:c6_usabilidad}): incorporar una gestión del historial de interacción que permita recuperar consultas anteriores.
  \item \textbf{Ampliación y robustez del estudio empírico.} Incrementar el tamaño de la muestra de evaluación y construir un conjunto de relevancia con anotación de oro que permita medir la calidad de recuperación en términos absolutos, no solo comparativos.
  \item \textbf{Evaluación del comportamiento de abstención.} Reintroducir en la muestra los casos sin evidencia, excluidos del estudio actual, para evaluar explícitamente la puerta de abstención cuando no existe respuesta esperable.
  \item \textbf{Recuperación sobre texto completo.} Extender el corpus más allá de los \textit{abstracts} hacia texto completo (por ejemplo, PubMed Central), evaluando el efecto sobre fidelidad y precisión de anclaje.
  \item \textbf{Soporte multilingüe.} Habilitar consulta y respuesta en español, manteniendo la trazabilidad de la evidencia de origen en inglés.
\end{itemize}
