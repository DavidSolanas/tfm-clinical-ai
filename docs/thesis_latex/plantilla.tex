...
\hline
RNF-07 & Calidad de la respuesta & La configuración híbrida completa (FT+RAG) debe superar a la configuración base en al menos $0{,}10$ puntos absolutos de \textit{Answer Relevancy} y mejorar la \textit{Faithfulness} respecto a la configuración RAG sin ajuste fino (C) ---único eje sobre el que esta métrica es computable, al requerir contexto recuperado--- sobre el \textit{benchmark} de evaluación RAGAS, según el objetivo cuantitativo relativo declarado en el Capítulo~3. \\
\hline
...
\textbf{Usabilidad: evaluación heurística del autor.} La validación cualitativa de usabilidad con un profesional sanitario quedó condicionada, ya en la Sección~\ref{sec:c4_validacion}, a la disponibilidad de dicha colaboración dentro del calendario del trabajo. Al no resultar posible, y conforme a la alternativa allí prevista, la usabilidad se valora mediante una revisión heurística documentada por el autor de la interfaz web (Gradio) y del flujo de interacción que esta expone, tomando como referencia un subconjunto de los principios heurísticos de usabilidad y anclando cada principio en funcionalidades del sistema efectivamente implementadas y evaluadas en los apartados anteriores. La Tabla~\ref{tab:c6_heuristica} resume esta valoración.
...
\hline
Visibilidad del estado del sistema & La interfaz muestra explícitamente la configuración activa (Base / FT / RAG / FT+RAG) y, junto a cada respuesta, los \textit{abstracts} recuperados con su PMID y un fragmento del texto (RF-09). \\
\hline
Control y libertad del usuario & El selector de configuración (RF-08) permite alternar entre las cuatro variantes y contrastar las respuestas para una misma consulta. \\
\hline
