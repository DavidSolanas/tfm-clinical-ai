...
% Permite a TeX estirar ligeramente el interlineado como último recurso
% para evitar pequeñas cajas desbordadas (overfull \hbox) en el texto corrido.
...
\chapter{Resumen}
El crecimiento sostenido del conocimiento biomédico supera la capacidad de asimilación del profesional clínico, y los Grandes Modelos de Lenguaje (LLM), pese a su potencial como interfaz con la literatura científica, presentan una tendencia a la alucinación inaceptable en el ámbito sanitario. Este trabajo diseña, implementa y evalúa un sistema híbrido de asistencia a la consulta bibliográfica clínica que combina el ajuste fino eficiente de un modelo de código abierto (LLaMA-3.1-8B mediante QLoRA, sobre transcripciones clínicas de MTSamples) con una arquitectura de Generación Aumentada por Recuperación (RAG) conectada a un corpus de unos 320.000 \textit{abstracts} de PubMed, indexado mediante búsqueda híbrida y \textit{embeddings} biomédicos. A través de un estudio ablativo de cuatro configuraciones (base, ajuste fino exclusivo, RAG exclusivo e híbrido completo), evaluado sobre métricas RAGAS y un conjunto de métricas deterministas de comportamiento de citación, se demuestra que el ajuste fino y la recuperación resuelven problemas distintos y complementarios: la configuración híbrida domina en todos los ejes medidos (fidelidad 0,656; relevancia de la respuesta 0,569; tasa de citación con PMID 0,98; precisión de anclaje 1,00). Se concluye que la recuperación aporta la evidencia verificable mientras que el ajuste fino disciplina su formato de citación, siendo ambos componentes necesarios y eficaces únicamente cuando operan de forma conjunta.

\vspace{1em}
\noindent{\bf Palabras Clave:} Grandes Modelos de Lenguaje; Generación Aumentada por Recuperación; Ajuste fino; Medicina basada en la evidencia; Alucinación.

\chapter{Abstract}
The sustained growth of biomedical knowledge exceeds the assimilation capacity of clinical practitioners, and Large Language Models (LLMs), despite their potential as an interface to the scientific literature, exhibit a tendency to hallucinate that is unacceptable in healthcare settings. This work designs, implements and evaluates a hybrid clinical literature-consultation assistance system that combines the efficient fine-tuning of an open-source model (LLaMA-3.1-8B via QLoRA, on MTSamples clinical transcriptions) with a Retrieval-Augmented Generation (RAG) architecture connected to a corpus of roughly 320,000 PubMed abstracts, indexed through hybrid search and biomedical embeddings. By means of an ablation study of four configurations (base, fine-tuning only, RAG only, and full hybrid), assessed with RAGAS metrics and a set of deterministic citation-behaviour metrics, it is shown that fine-tuning and retrieval solve distinct and complementary problems: the hybrid configuration dominates across every measured axis (faithfulness 0.656; answer relevancy 0.569; PMID citation rate 0.98; grounding precision 1.00). It is concluded that retrieval supplies the verifiable evidence while fine-tuning disciplines its citation format, with both components being necessary and effective only when operating jointly.

\vspace{1em}
\noindent{\bf Keywords:} Large Language Models; Retrieval-Augmented Generation; Fine-tuning; Evidence-based medicine; Hallucination.
...
\textbf{Usabilidad: evaluación heurística del autor.} La validación cualitativa de usabilidad con un profesional sanitario quedó condicionada, ya en la Sección~\ref{sec:c4_validacion}, a la disponibilidad de dicha colaboración dentro del calendario del trabajo. Al no resultar posible, y conforme a la alternativa allí prevista, la usabilidad se valora mediante una revisión heurística documentada por el autor sobre la interfaz web (Gradio) ya integrada, ilustrada en las Figuras~\ref{fig:c5_ui_landing}--\ref{fig:c5_ui_abstention}. La revisión toma como referencia un subconjunto de los principios heurísticos de usabilidad ampliamente aceptados y, para cada principio, emite una valoración (\textit{cumplido} o \textit{cumplido con reservas}) anclada en funcionalidades del sistema efectivamente implementadas y evaluadas en los apartados anteriores. La Tabla~\ref{tab:c6_heuristica} resume esta valoración.

\begin{longtable}{|p{0.22\textwidth}|p{0.50\textwidth}|p{0.16\textwidth}|}
...
\hline
\textbf{Principio heurístico} & \textbf{Soporte y observaciones} & \textbf{Valoración} \\
\hline
\endfirsthead
...
\hline
\endlastfoot
Visibilidad del estado del sistema & La interfaz muestra un indicador de estado del servicio (modelo de generación activo y disponibilidad de la recuperación) y, junto a cada respuesta, los \textit{abstracts} recuperados con su PMID y un fragmento del texto (RF-09; Figura~\ref{fig:c5_ui_answer}). Durante la generación, al lanzar la consulta la aplicación expone de forma inmediata un indicador de progreso animado acompañado de una estimación de la latencia esperada (del orden de 30--60~s), de modo que el usuario nunca permanece ante una espera silenciosa. & Cumplido \\
\hline
Control y libertad del usuario & El usuario reformula libremente la consulta en lenguaje natural y dispone de notas de ejemplo para iniciar el análisis (Figura~\ref{fig:c5_ui_landing}). Como reserva, la interfaz no conserva un historial de la interacción ni ofrece una acción explícita para deshacer o recuperar una consulta anterior. & Cumplido con reservas \\
\hline
Reconocer en lugar de recordar & Las fuentes se presentan junto a la respuesta y cada PMID es un enlace activo al registro de PubMed (Figuras~\ref{fig:c5_ui_answer} y~\ref{fig:c5_ui_pubmed}), de modo que el usuario verifica la evidencia sin abandonar la interfaz ni memorizar identificadores. & Cumplido \\
\hline
Prevención de errores & La puerta de abstención y el verificador de citaciones (Secciones~\ref{sec:c5_clinical_scorer} y~\ref{sec:c5_citation}) evitan que el sistema emita respuestas sin evidencia suficiente o con citas no ancladas en el contexto recuperado; el estado de abstención se comunica explícitamente al usuario (Figura~\ref{fig:c5_ui_abstention}). & Cumplido \\
\hline
Coincidencia con el dominio real & El ajuste fino adapta la terminología y la estructura de la respuesta (recomendación / base de evidencia / incertidumbre) al registro clínico, como evidencian las métricas de formato y citación de las Secciones anteriores. & Cumplido \\
\hline
\end{longtable}

\textbf{Resultado.} De los cinco principios revisados, cuatro se consideran plenamente satisfechos y uno satisfecho con reservas; no se identificaron defectos de usabilidad críticos que impidan el uso previsto de la herramienta. La única reserva se concentra en una dimensión bien delimitada y no esencial para la función central de consulta bibliográfica: la ausencia de una gestión del historial de interacción que permita recuperar consultas anteriores. Esta mejora se recoge como línea concreta de la interfaz en el Capítulo~\ref{cap:conclusiones}.

Esta evaluación heurística es una valoración del autor y no sustituye a un estudio empírico de usabilidad con usuarios reales. Sus limitaciones (ausencia de profesionales sanitarios y de métricas de interacción: tiempo de tarea, tasa de éxito, satisfacción percibida) se asumen explícitamente, y la realización de una sesión de evaluación con al menos un experto del dominio clínico, sobre la interfaz ya integrada, se identifica como línea prioritaria de trabajo futuro (Capítulo~\ref{cap:conclusiones}).
...
Dada la cantidad de variantes y de métricas analizadas en las secciones anteriores, conviene consolidar antes de discutirlas las cifras más destacadas en un único cuadro. La Tabla~\ref{tab:c6_resumen} reúne, para las cuatro configuraciones, las seis métricas que mejor sintetizan los dos ejes del estudio (calidad de respuesta y comportamiento de citación) y que sustentan las conclusiones del Capítulo~\ref{cap:conclusiones}; recoge, condensadas, las cifras dispersas en las Tablas~\ref{tab:c6_ragas},~\ref{tab:c6_citacion} y~\ref{tab:c6_robustez}.
...
\hline
\textbf{Métrica destacada} & \textbf{A (base)} & \textbf{B (FT)} & \textbf{C (RAG)} & \textbf{D (FT+RAG)} \\
\hline
\textit{Faithfulness} (fidelidad)        & ---    & ---    & 0{,}573 & \textbf{0{,}656} \\
\hline
\textit{Answer relevancy} (relevancia)   & 0{,}000 & 0{,}311 & 0{,}452 & \textbf{0{,}569} \\
\hline
Tasa de citación con PMID                & 0{,}00 & 0{,}04 & 0{,}84  & \textbf{0{,}98} \\
\hline
Precisión de anclaje de citaciones       & ---    & ---    & 1{,}00  & \textbf{1{,}00} \\
\hline
Marcador de posición (fabricación de forma) & 0{,}00 & 0{,}60 & 0{,}00 & \textbf{0{,}00} \\
\hline
Alucinación de PMID                      & 0{,}00 & 0{,}04 & 0{,}00 & 0{,}02 \\
\hline
\end{tabular}
\caption{Cuadro resumen de las métricas más destacadas por configuración ($n=50$). Para las dos primeras filas, mejor es mayor; para las dos últimas, mejor es menor. En negrita, el mejor valor por fila (en empate, la configuración~D). Las métricas dependientes de contexto solo aplican a C~y~D.}
\label{tab:c6_resumen}
\end{table}

La lectura conjunta del cuadro anticipa el argumento de la discusión: la configuración~D iguala o supera a las otras configuraciones en todas las métricas de calidad y de citación, y el único modo de fallo relevante (el marcador de posición del 60\,\% en~B) desaparece al añadir la recuperación.
...
Este capítulo cierra el trabajo recapitulando sus contribuciones, contrastando los resultados obtenidos con los objetivos planteados en el Capítulo~\ref{cap:objetivos}, asumiendo las limitaciones identificadas y proponiendo las líneas de continuación más prometedoras.

\section{Conclusiones generales}
\label{sec:c7_conclusiones}

El trabajo ha diseñado, implementado y evaluado un sistema híbrido de asistencia a la consulta bibliográfica clínica que combina el ajuste fino eficiente de un LLM de código abierto (LLaMA-3.1-8B-Instruct mediante QLoRA) con una arquitectura RAG conectada a literatura primaria de PubMed, todo ello sobre hardware modesto y bajo prácticas MLOps que garantizan la trazabilidad del ciclo experimental. El estudio ablativo de cuatro configuraciones (base, ajuste fino exclusivo, RAG exclusivo e híbrido completo) constituye la contribución empírica central y permite separar de forma limpia la aportación de cada componente.

La conclusión principal es que \textbf{el ajuste fino y la recuperación resuelven problemas distintos y complementarios}, y que su combinación (configuración~D) es la mejor en todos los ejes medidos: fidelidad (\textit{faithfulness} 0{,}656), relevancia de la respuesta (\textit{answer relevancy} 0{,}569), tasa de citación con PMID (0{,}98) y precisión de anclaje (1{,}00). La recuperación es la condición necesaria para que el sistema cite literatura real y para que la fidelidad sea siquiera computable; el ajuste fino disciplina el comportamiento del generador, corrigiendo el formato de citación hacia el patrón trazable \texttt{(PMID:~XXXXX)} y elevando la relevancia. Estos hallazgos son coherentes con la literatura comparativa de RAG frente a ajuste fino en dominios especializados \cite{ovadia2024finetuningretrievalcomparingknowledge, balaguer2024ragvsfinetuningpipelines, pingua_medical_FT_vs_RAG_2025} y la confirman en el escenario concreto de la asistencia bibliográfica clínica.

Un segundo hallazgo, de naturaleza más cautelar, es que \textbf{el ajuste fino sin recuperación es frágil ante la ausencia de evidencia}. La configuración~B, al operar fuera de su distribución de entrenamiento, reproduce el marcador de posición literal de la plantilla en el 60\,\% de sus respuestas en lugar de rehusar, un comportamiento que solo las métricas deterministas de citación diseñadas para este estudio logran capturar. Este resultado tiene una lectura directa de despliegue: un modelo ajustado no debe servirse en producción sin su pipeline de recuperación y sin las salvaguardas de puerta de abstención y verificación de citaciones \cite{ji_survey_2023, pal_med-halt_2023}.

\section{Grado de consecución de los objetivos}
\label{sec:c7_objetivos}

El balance global es claro: \textbf{se han cumplido la totalidad de los objetivos planteados en el Capítulo~\ref{cap:objetivos}}, tanto el objetivo general como los cinco objetivos específicos. El objetivo general se ha alcanzado al haberse construido y evaluado el sistema híbrido y haberse cuantificado la contribución diferencial de cada componente. La Tabla~\ref{tab:c7_objetivos} resume el grado de consecución de cada objetivo y la evidencia principal que lo respalda.
...
\hline
\textbf{Objetivo} & \textbf{Estado} & \textbf{Evidencia principal} \\
\hline
\textbf{OG}: Construir y evaluar el sistema híbrido y cuantificar la contribución de cada componente & Cumplido & Capítulos~\ref{cap:diseno} y~\ref{cap:evaluacion}: estudio ablativo de las cuatro configuraciones. \\
\hline
\textbf{OE1}: Pipeline de ingestión e indexación semántica & Cumplido & Corpus de \textit{abstracts} de PubMed dirigido por el árbol MeSH e indexado en Qdrant (denso~+~BM25, fusión RRF); embeddings Bioformer-16L seleccionados empíricamente (Secciones~\ref{sec:c5_embedding} y~\ref{sec:c5_rag}). \\
\hline
\textbf{OE2}: Ajuste fino con QLoRA sobre MTSamples & Cumplido & LLaMA-3.1-8B ajustado con QLoRA sobre un conjunto de instrucción-respuesta derivado de MTSamples vía \textit{teacher} local, adaptando estilo, terminología y estructura al registro clínico (Secciones~\ref{sec:c5_sft} y~\ref{sec:c5_qlora}). \\
\hline
\textbf{OE3}: Integración híbrida bajo MLOps & Cumplido & Arquitectura única con seguimiento de las cuatro configuraciones en MLflow y reproducibilidad procedimental (\texttt{seed=42}, \textit{hash} de dataset e hiperparámetros registrados). \\
\hline
\textbf{OE4}: Evaluación de las cuatro configuraciones & Cumplido & Capítulo~\ref{cap:evaluacion}: métricas RAGAS y métricas deterministas de citación sobre una muestra pareada de 50~consultas, con intervalos \textit{bootstrap}. \\
\hline
\textbf{OE5}: Determinar la ventaja del sistema híbrido & Cumplido & La configuración~D domina a las parciales en todos los ejes medidos; el análisis identifica qué limitación resuelve cada técnica (RAG: evidencia y trazabilidad; ajuste fino: formato y relevancia). \\
\hline
\end{tabular}
\caption{Grado de consecución del objetivo general (OG) y de los objetivos específicos (OE1--OE5).}
\label{tab:c7_objetivos}
\end{table}

En cuanto a los criterios cuantitativos de éxito asociados al requisito RNF-07, el criterio sobre \textit{answer relevancy} (que la configuración~D supere a la base en al menos $0{,}10$~puntos) \textbf{se cumple con amplio margen} ($+0{,}569$). El criterio sobre \textit{faithfulness} requiere la matización ya documentada en la Sección~\ref{sec:c6_discusion}: esta métrica no está definida para la configuración base (sin contexto recuperado), por lo que su único eje comparable es C$\to$D, sobre el que el ajuste fino mejora la fidelidad en $+0{,}083$~puntos. La dirección y el signo del efecto respaldan la hipótesis, si bien su magnitud queda ligeramente por debajo del umbral de $0{,}10$ que se barajó en una formulación inicial del criterio.

\section{Contribuciones del trabajo}
\label{sec:c7_contribuciones}

Las contribuciones concretas de este trabajo, en línea con la brecha identificada en la literatura (Sección~\ref{sec:c2_sintesis}), se articulan en torno a cuatro aportaciones. La primera, y de carácter central, es un estudio ablativo aplicado a la asistencia bibliográfica clínica sobre literatura primaria de PubMed, realizado íntegramente sobre un único modelo de código abierto cuantizado en hardware modesto, frente al patrón dominante de modelos propietarios y arquitecturas multi-LLM \cite{amugongo_retrieval_2025, zakka_almanac_2024, lozano_clinfoai_2023}. Sobre ese estudio se sostiene la segunda contribución: una familia de métricas deterministas de comportamiento de citación, libres de juez y reproducibles, que cuantifican la activación, el formato y el anclaje de las citaciones y que revelan modos de fallo (como la fabricación de la forma de una citación) invisibles tanto a la adherencia de formato como a las métricas RAGAS estándar. La tercera aportación es un mecanismo explícito de verificación automática de citaciones y de abstención, que permite medir directamente la contribución de RAG a la trazabilidad y a la contención de la alucinación de identificadores. La cuarta, finalmente, es una herramienta funcional integrada (una consola de evidencia clínica con interfaz web Gradio) que materializa la configuración híbrida completa para su uso interactivo.

\section{Limitaciones}
\label{sec:c7_limitaciones}

Más allá de las amenazas a la validez del estudio empírico ya detalladas en la Sección~\ref{sec:c6_limitaciones}, el sistema en su conjunto presenta cuatro limitaciones de alcance. La primera atañe a la cobertura del conocimiento: la base de evidencia se limita a \textit{abstracts} de PubMed y no a su texto completo, lo que acota la profundidad de la evidencia recuperable. La segunda concierne a la validación de usabilidad, resuelta mediante una revisión heurística del autor (Sección~\ref{sec:c6_usabilidad}) y, por tanto, carente de validación con profesionales sanitarios y de métricas de interacción. El idioma constituye la tercera restricción, pues el sistema opera en inglés, lo que limita su aplicabilidad directa en entornos clínicos hispanohablantes. La cuarta es de alcance deliberado: el sistema es una ayuda a la consulta bibliográfica y no un sistema de soporte a la decisión clínica, por lo que no debe interpretarse como sustituto del juicio profesional.

\section{Líneas de trabajo futuro}
\label{sec:c7_futuro}

Las limitaciones anteriores delimitan las líneas de continuación más prometedoras, que pueden agruparse en tres frentes. El frente prioritario es el de la usabilidad: realizar una sesión de evaluación con al menos un profesional del dominio clínico sobre la interfaz integrada, con métricas de interacción (tiempo de tarea, tasa de éxito y satisfacción percibida) que complementen la revisión heurística; ligado a ella, conviene subsanar la única reserva detectada en dicha revisión (Sección~\ref{sec:c6_usabilidad}) incorporando una gestión del historial de interacción que permita recuperar consultas anteriores.

En el frente empírico, la continuación natural pasa por reforzar el propio estudio ablativo: incrementar el tamaño de la muestra de evaluación y construir un conjunto de relevancia con anotación de oro que permita medir la calidad de recuperación en términos absolutos y no solo comparativos, así como reintroducir en la muestra los casos sin evidencia (excluidos del estudio actual) para evaluar de forma explícita el comportamiento de la puerta de abstención cuando no existe una respuesta esperable.

Por último, en el frente de la ampliación del alcance del sistema, dos extensiones resultan especialmente atractivas: llevar la recuperación más allá de los \textit{abstracts} hacia el texto completo (por ejemplo, PubMed Central), evaluando su efecto sobre la fidelidad y la precisión de anclaje, y habilitar el soporte multilingüe para consulta y respuesta en español, manteniendo la trazabilidad de la evidencia de origen en inglés.

\section{Reflexión final}
\label{sec:c7_reflexion}

El trabajo confirma, sobre un escenario clínico concreto y con recursos accesibles, una idea con implicaciones prácticas claras: en la asistencia bibliográfica clínica, la recuperación es la que aporta la evidencia verificable y el ajuste fino es el que disciplina la forma en que esa evidencia se comunica. Ninguno de los dos componentes es prescindible, y su valor solo se manifiesta plenamente cuando operan de forma conjunta y rodeados de salvaguardas explícitas de verificación y abstención. Es precisamente esa combinación, y no cada técnica por separado, la que acerca a un LLM de código abierto al estándar de trazabilidad que exige el dominio sanitario.
...
\chapter[Ejemplos cualitativos de respuestas por configuración]{Ejemplos cualitativos de\\ respuestas por configuración}
...
El código fuente completo del sistema descrito en esta memoria (pipeline de ingestión e indexación, módulo RAG, \textit{scripts} de ajuste fino y conversión, evaluación, interfaz web y ficheros de configuración) se encuentra disponible en el siguiente repositorio público:

\begin{center}
\url{https://github.com/DavidSolanas/tfm-clinical-ai}
\end{center}

El alumno en este Trabajo de Fin de Estudios es el único autor y responsable de la totalidad de las contribuciones (\textit{commits}) registradas en el repositorio.
