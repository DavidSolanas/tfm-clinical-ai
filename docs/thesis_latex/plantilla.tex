...
La Tabla~\ref{tab:c5_qlora} resume los hiperparámetros de entrenamiento, definidos en \texttt{configs/training.yaml}.
...
El entrenamiento se realiza mediante \texttt{SFTTrainer} de TRL (\textit{Transformer Reinforcement Learning}), aplicando el \textit{chat template} de LLaMA-3.1 para formatear los ejemplos antes de la tokenización. Los parámetros, hiperparámetros, pérdida de entrenamiento y pérdida de validación por paso se registran automáticamente en MLflow mediante \texttt{report\_to="mlflow"} (RF-11). El adaptador LoRA resultante se guarda en \texttt{checkpoints/llama-3.1-8b-mtsamples-qlora/} como un conjunto de pesos ligero que puede cargarse de forma independiente sobre cualquier instancia del modelo base cuantizado sin modificar sus pesos originales, permitiendo alternar entre las configuraciones FT y FT+RAG cargando o descargando el adaptador en tiempo de ejecución.
...
La interfaz web del sistema se implementa mediante Gradio, satisfaciendo el requisito RF-12. La interfaz permite al usuario: (a)~formular una consulta clínica en lenguaje natural, (b)~seleccionar la configuración del sistema entre las cuatro variantes ablativas (RF-08), y (c)~visualizar la respuesta estructurada junto con los abstracts de PubMed recuperados, incluyendo el PMID y un fragmento del texto de cada abstract (RF-09). La selección de configuración activa o desactiva el pipeline RAG y el cargado del adaptador LoRA de forma transparente para el usuario.

\noindent\textbf{Estado de implementación:} la interfaz se encuentra en fase de integración final al cierre de la presente memoria. Los resultados de evaluación del Capítulo~6 se obtienen invocando el pipeline directamente desde código, sin depender de la interfaz, por lo que su estado no condiciona la validez del estudio ablativo. La validación de usabilidad (RF-12, RNF-06) se realizará una vez completada la integración, en los términos descritos en la Sección~4.4.
